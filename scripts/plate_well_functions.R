...
plate_tile_plot <- function(data_frame, summary_results, palette = "Reds",
                            plate_type = "96") {
  # check data_frame is a data.frame and check that it has the right columns
  if (class(data_frame) != "data.frame") {
    stop("Supplied object is not a data frame")
  }
  required_columns <- c("plex", "plate", "group_name", "row_i", "col",
                        "pc_reads", "total_on_target_reads")
  missing_columns <- setdiff(required_columns, colnames(data_frame))
  if (length(missing_columns) > 0) {
    stop(paste("Required columns are missing '",
               paste0(missing_columns, collapse = ", "), "'.", sep = ""))
  }
  ...
  total_reads_text <- subset(data_frame,
                             select = c(col, row_i, total_on_target_reads))
  total_reads_text$total_on_target_reads <-
    as.character(total_reads_text$total_on_target_reads)
  ...
  for (column in 1:last_col) {
    for (row_num in -1:-(last_row_i)) {
      if (nrow(data_frame[data_frame$col == column &
                            data_frame$row_i == row_num, ]) == 0) {
        new_row <- data.frame(
          plex = data_frame$plex[1],
  ...
  # create title text
  title_text <- paste("Number of indels per embryo (",
                      data_frame$plex[1], " - plate ", data_frame$plate[1],
  ...
  row_breaks <- seq(-1, -last_row_i)
  col_breaks <- seq(1, last_col)
  # make plot
  plate_plot <- ggplot(data_frame, aes(x = col, y = row_i)) +
    geom_tile(aes(fill = pc_reads)) +
    scale_fill_continuous(low = plot_colours[1], high = plot_colours[2],
                          space = "Lab", na.value = "grey90",
                          guide = "colourbar", limits = scale_limits) +
    geom_text(data = pc_reads_text, aes(label = pc_reads),
              size = pcs_text_size) +
    geom_text(data = total_reads_text, aes(label = total_on_target_reads),
              size = reads_text_size) +
